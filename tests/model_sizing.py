...
def calculate_model_params(
    config: ModelConfig, use_swiglu=True, use_rope=True, use_rmsnorm=True
) -> int:
...
def find_optimal_config(
        ...
        for num_heads in [4, 8, 12, 16]:
            ...
            for ff_ratio in [2, 3, 4]:
                ...
                for num_layers in range(
                    max(1, estimated_layers - 2), estimated_layers + 3
                ):
                    config = ModelConfig(
    ...
    actual_params = calculate_model_params(
        best_config, use_swiglu=True, use_rope=True, use_rmsnorm=True
    )
    print(f"\nTarget parameters: {target_params:,}")
    print(f"Actual parameters: {actual_params:,}")
    print(
        f"Difference: {abs(actual_params - target_params):,} ({abs(actual_params - target_params) / target_params * 100:.2f}%)"
    )
    print("\nModel configuration:")
...
def create_model_from_params(
    target_params: int, vocab_size: int, max_len: int = 512, **kwargs
) -> ModelConfig:
    """
