...
def test_memory_issue():
    ...
    print(f"Parameters: seq_len={seq_len}, stride={stride}, max_examples={max_examples}")
    ...
    print(f"   Memory: {mem_after_texts:.1f} MB (+{mem_after_texts - mem_before:.1f} MB)")
    ...
    print(f"   Memory: {mem_after_tokens:.1f} MB (+{mem_after_tokens - mem_after_texts:.1f} MB)")
    
    # Step 3: Calculate expected sequences
    num_sequences = (len(all_tokens) - seq_len) // stride + 1
    expected_memory_mb = num_sequences * seq_len * 2 * 4 / 1024 / 1024  # 2 arrays, 4 bytes per int32
    print(f"\n4. Sequence analysis:")
    ...
    for i in range(0, len(all_tokens) - seq_len, stride):
        ...
        if i % 1000 == 0 and i > 0:
            ...
            print(f"   Progress: {i//stride}/{num_sequences} sequences, Memory: {current_mem:.1f} MB")
    
    mem_after_sequences = get_memory_mb()
    print(f"   Final memory: {mem_after_sequences:.1f} MB (+{mem_after_sequences - mem_after_tokens:.1f} MB)")
    ...
    print(f"Actual vs expected:     {(mem_after_sequences - mem_after_tokens) / expected_memory_mb:.1f}x")
    
    # Identify the problem
    print(f"\nPROBLEM IDENTIFIED:")
    print(f"1. Overlapping sequences create duplicate data storage")
    print(f"2. Each sequence stores a full copy of tokens")
