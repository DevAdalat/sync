...
def test_real_memory_fix():
    ...
    print(f"Memory-efficient version:")
    ...
    if hasattr(inputs_eff, 'shape') and hasattr(inputs_orig, '__len__'):
        ...
        if len(inputs_eff) > 0 and len(inputs_orig) > 0:
            ...
            print(f"  - First sequence matches: {np.array_equal(eff_first, orig_first)}")
    ...
    return {
        ...
        'reduction': mem_original_used / mem_efficient_used if mem_efficient_used > 0 else 1
    }
