...
from model_sizing import calculate_model_params, create_model_from_params
...
def test_model(target_params, vocab_size=256, max_len=100, batch_size=4, seq_len=32):
    ...
    theoretical_params = calculate_model_params(
        config, use_swiglu=True, use_rope=True, use_rmsnorm=True
    )
    ...
    print(
        f"Difference from target: {abs(actual_params - target_params):>15,} ({abs(actual_params - target_params) / target_params * 100:>6.2f}%)"
    )
    print(f"{'=' * 70}")
    ...
    def loss_fn(params, inputs, labels):
        ...
        loss = -jnp.mean(
            jnp.sum(jax.nn.one_hot(labels, vocab_size) * log_probs, axis=-1)
        )
        return loss
...
def main():
    ...
    parser.add_argument(
        "--test-all", action="store_true", help="Test all predefined sizes (1M, 3M, 5M)"
    )
    ...
    if args.test_all:
        ...
        for size in test_sizes:
            result = test_model(
                target_params=size, vocab_size=args.vocab_size, max_len=args.max_len
            )
        ...
        print("=" * 70)
        print(
            f"{'Target':<12} {'Theoretical':<15} {'Actual':<15} {'Difference':<12} {'Error %':<10}"
        )
        print("-" * 70)
        for r in results:
            ...
            print(
                f"{r['target_params']:>11,} {r['theoretical_params']:>14,} {r['actual_params']:>14,} {diff:>11,} {error_pct:>9.2f}%"
            )
        print("=" * 70)
