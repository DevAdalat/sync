...
def test_memory_fix_simple():
    ...
    print(
        f"   Memory usage: ~{len(inputs_old) * seq_len * 2 * 8 / 1024:.1f} KB (Python lists)"
    )
    ...
    print(
        f"   Memory usage: ~{len(inputs_new) * seq_len * 2 * 4 / 1024:.1f} KB (numpy arrays)"
    )
    ...
    print(
        f"   Memory usage: ~{len(inputs_jax) * seq_len * 2 * 4 / 1024:.1f} KB (JAX arrays)"
    )
