...
class StreamingDataLoader:
    ...
    def __init__(
        ...
        logger.info(f"Initializing StreamingDataLoader")
...
class MemoryEfficientDataLoader:
    ...
    def __init__(
        ...
        """
        self.dataset_id = dataset_id
        self.tokenizer = tokenizer if isinstance(tokenizer, Tokenizer) else Tokenizer.from_file(tokenizer)
        self.seq_len = seq_len
        self.batch_size = batch_size
        self.text_column = text_column
        self.stride = stride if stride else seq_len
        ...
        logger.info(f"Initializing MemoryEfficientDataLoader")
        logger.info(f"  Chunk size: {chunk_size} sequences")
        logger.info(f"  Memory usage: ~{chunk_size * seq_len * 4 / 1024 / 1024:.1f} MB per chunk")
...
if __name__ == "__main__":
    ...
    print(f"  100K sequences, seq_len=128, batch_size=32")
