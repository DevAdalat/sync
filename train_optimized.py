...
def print_memory_info():
    ...
    if backend == "gpu":
        ...
        print(f"  TPU detected - HBM memory optimizations active")
    else:
        print(f"  CPU backend - system RAM usage")
...
def train_model_optimized(
    ...
    print("="*80)
    print(f"Streaming data loading:      {'✓ ENABLED' if use_streaming else '✗ Disabled'}")
    print(f"Gradient accumulation:       {gradient_accumulation_steps}x steps")
    print(f"Mixed precision training:    {'✓ ENABLED (bfloat16)' if use_mixed_precision else '✗ Disabled (float32)'}")
    print(f"8-bit optimizer states:      {'✓ ENABLED' if use_8bit_optimizer else '✗ Disabled'}")
    ...
    optimizer_memory_mb = param_memory_mb * 2  # AdamW stores 2 states (momentum, variance)
    ...
    if use_streaming:
        ...
        print(f"  No full dataset loading - data streamed on-demand!")
    ...
    if estimated_batches_per_epoch:
        total_steps = (estimated_batches_per_epoch // gradient_accumulation_steps) * epochs
    else:
    ...
    def loss_fn(params, batch, dropout_rng):
        ...
        logits = model.apply(params, inputs, deterministic=False, rngs={'dropout': dropout_rng})
