...
def load_tokenizer(
    path: Optional[str] = None, pretrained: Optional[str] = None
) -> Tokenizer:
...
def upload_checkpoint_to_cloud(
    local_path: str, cloud_config: CloudConfig, checkpoint_name: str
) -> str:
...
def download_checkpoint_from_cloud(
    cloud_path: str, cloud_config: CloudConfig, local_path: str
) -> str:
...
def _download_from_s3(
    ...
    key = cloud_path.replace(f"s3://{cloud_config.bucket_name}/", "").replace(
        ".zip", ""
    )
    zip_key = f"{key}.zip"
...
def _upload_to_gcs(
    ...
    try:
        ...
        raise ImportError(
            "google-cloud-storage is required for GCS uploads. Install with: pip install google-cloud-storage"
        )
...
def _download_from_gcs(
    ...
    try:
        ...
        raise ImportError(
            "google-cloud-storage is required for GCS downloads. Install with: pip install google-cloud-storage"
        )
    ...
    # Extract key from cloud_path
    key = cloud_path.replace(f"gs://{cloud_config.bucket_name}/", "").replace(
        ".zip", ""
    )
    zip_key = f"{key}.zip"
...
def _upload_to_azure(
    ...
    try:
        ...
        raise ImportError(
            "azure-storage-blob is required for Azure uploads. Install with: pip install azure-storage-blob"
        )

    if not cloud_config.sas_token or not cloud_config.account_name:
        raise ValueError(
            "Azure SAS token and account name are required for Azure uploads"
        )
...
def _download_from_azure(
    ...
    try:
        ...
        raise ImportError(
            "azure-storage-blob is required for Azure downloads. Install with: pip install azure-storage-blob"
        )

    if not cloud_config.sas_token or not cloud_config.account_name:
        raise ValueError(
            "Azure SAS token and account name are required for Azure downloads"
        )
    ...
    if "?" in cloud_path:
        ...
        blob_path = cloud_path.replace(
            f"https://{cloud_config.account_name}.blob.core.windows.net/{cloud_config.bucket_name}/",
            "",
        )
