...
class MemoryEfficientDatasetLoader:
    ...
    def stream_tokens(
        self, tokenizer: Union[Tokenizer, str], max_examples: Optional[int] = None
    ...
    def prepare_sequences_memory_efficient(
        ...
        logger.info(
            f"Memory-efficient sequence preparation (seq_len={seq_len}, stride={stride})"
        )
...
def compare_memory_usage():
    ...
    print(
        f"   Original method memory: {mem_after_orig:.2f} GB (+{mem_after_orig - mem_before:.2f} GB)"
    )
    print(f"   Sequences created: {len(inputs_orig)}")

    # Store lengths before cleanup
    len_orig = len(inputs_orig)
    ...
    print(
        f"   Efficient method memory: {mem_after_eff:.2f} GB (+{mem_after_eff - mem_before:.2f} GB)"
    )
    print(f"   Sequences created: {len(inputs_eff)}")

    # Store length for efficient method
    len_eff = len(inputs_eff)
    ...
    print(
        f"Memory reduction:    {((mem_after_orig - mem_before) / (mem_after_eff - mem_before)):.1f}x"
    )
    print(f"Same sequences:      {len_orig == len_eff}")

    return {
        "original_memory": mem_after_orig - mem_before,
        "efficient_memory": mem_after_eff - mem_before,
        "reduction_factor": (mem_after_orig - mem_before)
        / (mem_after_eff - mem_before),
    }
